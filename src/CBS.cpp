...
void CBS::savePaths(const string &fileName) const
{
    ...
    size_t max_len = 0;
    for (int i = 0; i < num_of_agents; i++)
        max_len = std::max(max_len, paths[i]->size());

    // Output per-timestep in mapf-visualizer format: "t:(col0,row0),(col1,row1),..."
    // mapf-visualizer expects (x, y) = (col, row)
    for (size_t t = 0; t < max_len; t++)
    {
        ...
        for (int i = 0; i < num_of_agents; i++)
        {
            ...
            size_t idx = (t < path.size()) ? t : path.size() - 1;
            output << "(" << search_engines[0]->instance.getColCoordinate(path[idx].location)
                   << "," << search_engines[0]->instance.getRowCoordinate(path[idx].location) << ")";
